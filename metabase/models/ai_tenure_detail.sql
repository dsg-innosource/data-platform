-- -------------------------------------------------------------------------------------------
-- AI Tenure Detail - Multi-Pass ADP + AI Score Matching
-- -------------------------------------------------------------------------------------------
-- METABASE URL: https://innosource.metabaseapp.com/model/288-ai-tenure-detail
-- DESCRIPTION: Combines ADP employment data with AI assessment scores (Conversation and
--              Resume) using a multi-pass matching strategy:
--              - Pass 1: Initial hires with direct requisition matches
--              - Pass 2: Initial hires through pipeline requisitions (temporal matching)
--              - Pass 3: All rehires using temporal matching approach
-- DEPENDS ON: DP-000 (department code mapping for authoritative client resolution)
...
WITH adp_tenure AS (
    ...
    combined_results AS (
        SELECT
            ...
            final_requisition_id AS calc_requisition_id,
        ...
        SELECT
            ...
            calc_hire_date, calc_termination_date, tenure_days,
            jakib_score, resume_only_score,
            pipeline_req_used AS calc_requisition_id,
            final_requisition_key AS calc_requisition_key,
            auth_client_name AS calc_client, -- Use authoritative client name
            use_jakib_ai, pass_source
        FROM pass2_results
        ...
        SELECT
            ...
            calc_hire_date, calc_termination_date, tenure_days,
            jakib_score, resume_only_score,
            rehire_req_used AS calc_requisition_id,
            final_requisition_key AS calc_requisition_key,
            auth_client_name AS calc_client, -- Use authoritative client name
            use_jakib_ai, pass_source
        FROM pass3_results
    ...
    SELECT
        applicant_id, adp_id, payroll_name, is_active,
        calc_hire_date, calc_termination_date, tenure_days,
        jakib_score, resume_only_score, calc_requisition_id,
