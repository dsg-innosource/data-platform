-- -------------------------------------------------------------------------------------------
-- Requisition Application Progress with Recruiter Names
-- -------------------------------------------------------------------------------------------
...
,req_applicants as (
  ...
  from req_applicant_list ra
    left join bronze.portal_jakib_results jr on
      ra.requisition_id = jr.requisition_id
      and ra.applicant_id = jr.applicant_id
    left join bronze.portal_resume_scores rs on
...
recruiters as (
  ...
  from bronze.portal_users p
),
applicant_views as (
  select distinct on (av.applicant_id, av.requisition_id)
    av.applicant_id,
    av.requisition_id,
...
phone_screens as (
  select distinct on (rs.requisition_id, rs.applicant_id)
    rs.requisition_id,
    rs.applicant_id,
  ...
  where
    ...
    and extract ('Year' from rs.created_at) >= 2024
  order by
    rs.requisition_id,
    rs.applicant_id,
    rs.created_at
...
hires as (
  ...
  where
    extract ('Year' from jo.created_at) >= 2024
  ...
  select
    ...
    case
      when av.first_applicant_view_date is not null then
        case
          when av.first_applicant_view_date >= coalesce(ra.score_created_at, ra.resume_only_score_created_at, av.first_applicant_view_date) then false
          else true
        end
      else false
